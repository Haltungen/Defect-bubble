...
func1 = @(x1,x2) ops.GBiPeriodic(k1, x1-y(1), x2-y(2), d, d, alpha);%-ops.GBiPeriodic(k2, x1-y(1), x2-y(2), d, d, alpha);
func2 = @(x1,x2) -1i/4*besselh(0,1,k1*sqrt((x1-y(1)).^2+(x2-y(2)).^2));
func3 = @(x1,x2) dot([cos(atan2(x2,x1));sin(atan2(x2,x1))],ops.GradGBiPeriodic(k1, x1-y(1), x2-y(2), d, d, alpha));
...
x1 = R_b*cos(theta);
x2 = R_b*sin(theta);
G_ewald = ops.GBiPeriodic(k1, x1-y(1), x2-y(2), d, d, alpha);%-ops.GBiPeriodic(k2, x1-y(1), x2-y(2), d, d, alpha);
G_ewald_nu = dot([cos(theta);sin(theta)],ops.GradGBiPeriodic(k1, x1-y(1), x2-y(2), d, d, alpha));
