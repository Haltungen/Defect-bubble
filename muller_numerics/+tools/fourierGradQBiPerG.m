function out = fourierGradQBiPerG(r, y, alp, k, NN, N, M) 
%% fourierQBiPerG
%
% Overview:
%   Returns the Fourier coefficients of the normal derivative of the quasi-
%   biperiodic function \Gamma_\alpha^k(x,y) as as a function of x on a circle of radius r.
%
% Input:
...
for n = -NN:NN
    ...
    out(NN+1+n) = 0.5*k*out(NN+1+n)*(besselj(n-1,k*r) - besselj(n+1,k*r));
    if r_2 > r
        out(NN+1+n) = out(NN+1+n) + 0.5*k*sgn*besselh(-n,k*r_2)*exp(-1i*n*theta_2)*(besselj(n-1,k*r) - besselj(n+1,k*r));
    else
        out(NN+1+n) = out(NN+1+n) + 0.5*k*sgn*(besselh(n-1,k*r) - besselh(n+1,k*r))*besselj(-n,k*r_2)*exp(-1i*n*theta_2);
    end
end
out = -1i/4*out;
end